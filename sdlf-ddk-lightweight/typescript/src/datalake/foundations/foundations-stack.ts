...
export class FoundationsStack extends BaseStack {
  ...
  constructor(scope: Construct, id: string, props: FoundationsStackProps) {
    ...
    this.lakeformationBucketRegistrationRole =
      this.createLakeformationBucketRegistrationRole();
    ...
    const [analyticsBucket, analyticsBucketKey] =
      this.createBucket('analytics');
    this.analyticsBucket = analyticsBucket;
    this.analyticsBucketKey = analyticsBucketKey;
    const [artifactsBucket, artifactsBucketKey] =
  ...
  private createOctagonTable(props: createOctagonTableProps): dynamo.Table {
    ...
    const tableKey = new kms.Key(this, `${props.name}-table-key`, {
    ...
    new ssm.StringParameter(this, `${props.name}-table-name-ssm`, {
      parameterName: `/SDLF/DynamoDB/${tableName}`,
      stringValue: props.name
    });
    return new dynamo.Table(this, `${props.name}-table`, {
  ...
  private createLakeformationBucketRegistrationRole(): iam.Role {
    return new iam.Role(this, 'lakeformation-bucket-registration-role', {
  ...
  protected createSdlfGlueArtifacts(): iam.IRole {
    ...
    new s3Deployment.BucketDeployment(
      ...
      {
        sources: [
          s3Deployment.Source.asset(path.join(__dirname, '../src/glue/'))
        ],
    ...
    new iam.ManagedPolicy(this, `${this.resourcePrefix}-glue-job-policy`, {
      roles: [glueRole],
          ...
          new iam.PolicyStatement({
            ...
            conditions: {
              'ForAnyValue:StringLike': {
                'kms:ResourceAliases': `alias/${this.resourcePrefix}-*-key`
              }
            }
          }),
          new iam.PolicyStatement({
