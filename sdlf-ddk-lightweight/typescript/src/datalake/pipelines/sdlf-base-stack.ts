...
export class SDLFBaseStack extends BaseStack {
  ...
  constructor(scope: Construct, id: string, props: SDLFBaseStackProps) {
    ...
    const customerConfigs: any = JSON.parse(
      readFileSync('./src/datalake/pipelines/parameters.json', 'utf-8')
    );
    ...
    this.foundationsStage = new FoundationsStack(this, 'foundation-stage', {
    ...
    customerConfigs[props.environmentId].forEach((customerConfig: any) => {
      ...
      if (!datasetNames.includes(datasetName)) {
        ...
        pipeline.registerDataset(dataset, customerConfig.config ?? {});
      }
    });
  }
  ...
  protected createDatalakeLibraryLayer(): void {
    const datalakeLibraryLayer = new lambda.LayerVersion(
      ...
      'data-lake-library-layer',
      {
        ...
        code: lambda.Code.fromAsset(
          path.join(__dirname, '../src/layers/data_lake_library')
        ),
    ...
    new ssm.StringParameter(this, `data-lake-library-layer-ssm`, {
