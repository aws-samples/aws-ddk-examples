...
class MetricAPI:
    ...
    def _create_single_metric(self, metric_rec: MetricRecordInfo, value: int):
        ...
        self._process_sns_notifications(
            metric_rec, new_metric_value, new_metric_version
        )

        return True

    def _process_sns_notifications(
        self, metric_rec: MetricRecordInfo, new_metric_value: int, new_metric_version
    ):
        for metric_config_info in self.client.config.metric_info:
            if (
                ...
                if self._check_metric_threshold(
                    new_metric_value,
                    metric_config_info.evaluation,
                    metric_config_info.threshold,
                ) and (
                    (metric_config_info.notify == METRIC_ALWAYS)
                    or (
                        metric_config_info.notify == METRIC_ONCE
                        and not self._is_notification_sent(metric_rec)
                    )
                ):
                    message = {
                    ...
                    if self.client.is_sns_set():
                        ...
                        self.logger.warn(
                            "SNS ARN is not defined neither globally nor in the metrics"
                        )
                        return True
    ...
    def _get_metric_records(self, date_str, metric_name) -> [MetricRecordInfo]:
        ...
        for metric in metric_arr:
            # Root
            root_metric = MetricRecordInfo(
                root=metric, metric=metric, metric_type=METRIC_ROOT
            )
            records.append(root_metric)

            # Yearly
            year_suffix = ".Y" + date_str[:4]
            yearly_metric = MetricRecordInfo(
                root=metric, metric=metric + year_suffix, metric_type=METRIC_YEARLY
            )
            records.append(yearly_metric)

            # Monthly
            month_suffix = ".M" + date_str[:7]
            monthly_metric = MetricRecordInfo(
                root=metric, metric=metric + month_suffix, metric_type=METRIC_MONTHLY
            )
            records.append(monthly_metric)

            # Daily
            day_suffix = ".D" + date_str
            daily_metric = MetricRecordInfo(
                root=metric, metric=metric + day_suffix, metric_type=METRIC_DAILY
            )
    ...
    def get_metrics_value(self, metric):
        ...
        result = self.metrics_table.get_item(
            Key={"root": root, "metric": metric}, ConsistentRead=True
        )
        if "Item" in result:
            return self.metrics_table.get_item(
                Key={"root": root, "metric": metric}, ConsistentRead=True
            )["Item"]["value"]
        else:
            return 0
    ...
    def _send_sns_message(self, message, topic_arn):
        self.logger.debug(
            f"Send message to SNS: Message {message}, topicArn: {topic_arn}"
        )
        sns = self.client.sns
    ...
    def _get_topic_arn(self, name):
        if "arn:aws:sns:" in name:
            ...
            return ":".join(
                ["arn", "aws", "sns", self.client.region, self.client.account_id, name]
            )
