...
class S3Interface:
    def __init__(self, log_level=None, s3_client=None, s3_resource=None):
        ...
        self._s3_resource = s3_resource or boto3.resource(
            "s3", config=self._session_config
        )
    ...
    def upload_object(self, object_path, bucket, key, kms_key=None):
        ...
        try:
            ...
            self._s3_client.upload_file(
                object_path, bucket, key, ExtraArgs=extra_kwargs
            )
        except ClientError:
    ...
    def copy_object(
        self, source_bucket, source_key, dest_bucket, dest_key=None, kms_key=None
    ):
    ...
    def tag_object(self, bucket, key, tag_dict):
        ...
        try:
            self._s3_client.put_object_tagging(
                ...
                Key=key,
                Tagging={
                    "TagSet": [{"Key": k, "Value": v} for k, v in tag_dict.items()]
                },
            )
        except ClientError:
    ...
    def delete_objects(self, bucket, prefix):
        ...
        self._logger.info(
            "Successfully deleted all objects in bucket {} with prefix {}".format(
                bucket, prefix
            )
        )
    ...
    def get_last_modified(self, bucket, key):
        last_modified_date = self._s3_client.head_object(Bucket=bucket, Key=key)[
            "LastModified"
        ]
