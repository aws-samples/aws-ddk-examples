...
def lambda_handler(event, context):
    ...
    try:
        ...
        event = event["Payload"]
        processed_keys = event["body"]["processedKeys"]["Payload"]
        ...
        sqs_config = SQSConfiguration(
            team, dataset, "".join([stage[:-1], chr(ord(stage[-1]) + 1)])
        )
        ...
        try:
            ...
            logger.info(
                "Next stage either does not exist or does not have an associated SQS queue."
            )
